...
<div class="content">
	<div class="sub-box">
		...
		<div class="sub-right" id="tf">
			<input type="hidden" id="area"> <input type="text" value="" name="name" class="sub-search" placeholder="请输入姓名" id="find" onkeyup="findSale($('#area').val())" /> <a id="aid" charset='ISO-8859-1' href="###" class="sub-button">添加</a>
		</div>
	</div>
			...
			if (res.length > 0) {
				...
				for (var i = 0; i < res.length; i++) {
					data[i] = {
						...
						name : res[i].name,
						age : res[i].phone,
						address : getTime(res[i].create_time),
						操作 : "<a href='toupdatesalesMan.action?id=" + res[i].id
								+ "'><img src='images/editors.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='images/deletes.png' onclick='deleted(" + res[i].id
								+ ")'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + updates(res[i].state, res[i].id)
					};
				}
								...
								if (res.length > 0) {
									...
									for (var i = 0; i < res.length; i++) {
										data[i] = {
											...
											address : getTime(res[i].create_time),
											操作 : "<a href='toupdatesalesMan.action?id=" + res[i].id
													+ "'><img src='images/editors.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='images/deletes.png' onclick='deleted("
													+ res[i].id + ")'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + updates(res[i].state, res[i].id)
										};
									}
