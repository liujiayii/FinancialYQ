			...
			<form action="##" method="post" enctype="multipart/form-data" id="save">
				<table class="control_tab">
					<tr>
						...
						<input type="hidden" value="${salaryVo.month}" name="month" >
...
<script type="text/javascript">
	...
	function update(){
		alert("123")
		var sum=nub('wages')/30*nub('attendance')+nub('wagess')/30*(nub('zattendance')+nub('maternity')+nub('marriage')+nub('pmaternity')+nub('annualleave'))+nub('shares')+nub('fulltimes')+nub('forck_bouns')+nub('meals')+nub('surance')+nub('deduct')+nub('licai_deduct')-nub('Notclock')-nub('insurance')-nub('fine')-nub('late')*10;
	if(sum>=5000){
