...
//			var not_lock = "<a href='toLockUser.action?id="+res[i]/.id+"'><img src='images/not_lock.png'></a>";
			...
			data[i] = {id:res[i].id,role_type:res[i].role_type,name:res[i].name,username:res[i].username,phone:res[i].phone,创建时间:getTime(res[i].create_time),修改时间:getTime(res[i].update_time),操作:toEdit+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+toDelete+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+toLock};
		}
	  	console.log(data)
		var cs = new table({
			"currentPageNum":2,
