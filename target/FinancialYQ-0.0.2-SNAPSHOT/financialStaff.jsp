...
<jsp:include page="nav.jsp"></jsp:include>
			<div class="content">
				<div class="sub-box">
					...
					<div class="sub-right">
						<input type="text" value="" name="name" class="sub-search" placeholder="请输入电话" id="inputPhone" onkeyup="findStaff()"/>
						<input type="button" value="添加" class="sub-button" onclick="window.location.href='toShowArea.action'"/>
					</div>
				</div>
		...
		$(function(){
			...
			$.getJSON(url,function(res){
				...
				for(var i=0;i<res.length;i++){
				   ...
				   data[i] = {id:res[i].id,job_number:res[i].job_number,name:res[i].name,area:res[i].area,dep:res[i].dep_name,post:res[i].post_name,phone:res[i].phone,entry_date:getTime(res[i].entry_date),操作:"<a href='toUpdateStaff.action?id="+res[i].id+"'><img src='images/editors.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;"+toLock(res[i].type,res[i].id)};
