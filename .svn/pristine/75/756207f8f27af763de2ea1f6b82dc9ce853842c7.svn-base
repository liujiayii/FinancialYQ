			...
			<div class="content">
				<div class="sub-box">
					<div class="sel-list">
						...
						<select class="select_style" id="monthid" onchange="getlist()">
				...
				<p class="money_statistical">合计：新标总金额为<span id="one"></span>元
				续贷总金额为<span id="two"></span>元</p>
			</div>
	...
	function findDeduct(){
		...
		$.getJSON(url,function(res){
			var data = [];
		  	...
		  	for(var i=0;i<res.length;i++){
			    data[i] = {id:res[i].id,职位:res[i].duty,姓名:res[i].name,电话:res[i].phone,提成:res[i].deduct==null?0:res[i].deduct,操作:res[i].duty=='业务员'?"<a href='showDetail.action?id="+res[i].id+"&time="+res[i].time+"'>详情</a>":""};
			}
		  	console.log(data)
			var cs = new table({
			    "tableId":"cs_table",  //必须
		...
		function getlist(){
			...
			$.getJSON(url,function(res){
				var data = [];
			for(var i=0;i<res.length;i++){
					    data[i] = {idss:res[i].id,name:res[i].duty,gender:res[i].name,age:res[i].phone,address:res[i].extract,操作:res[i].duty=='业务员'?"<a href='showDetail.action?id="+res[i].id+"&time="+res[i].time+"'>详情</a>":""};
				...
				$.getJSON(url,function(res){
					...
					if(res.length>0){
						for(var i=0;i<res.length;i++){
						    ...
						    data[i] = {id:res[i].id,职位:res[i].duty,姓名:res[i].name,电话:res[i].phone,提成:res[i].extract,操作:res[i].duty=='业务员'?"<a href='showDetail.action?id="+res[i].id+"&time="+res[i].time+"'>详情</a>":""};
						}
					}
				  	console.log(data)
					var cs = new table({
