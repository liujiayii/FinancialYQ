			...
			<div class="content">
				<div class="sub-box">
					...
					<%-- 	<select class="select_style" id="sid" onchange="getlist(this)">
						...
						<select class="select_style" id="monthid" onchange="getlist(this)">
				...
				<p class="money_statistical control_tab"><button style="float:right" id="export" class="bgbtn">导出</button></p>
			</div>
	...
	/*$('#select_type').change(function(){
		...
		function getlist(){
			...
			$.getJSON(url,function(res){
				var data = [];
			for(var i=0;i<res.length;i++){
			    data[i] = {id:res[i].id,职位:res[i].post_name,姓名:res[i].name,基本工资:res[i].job_number,实发工资:res[i].real_salary,操作:"<a href='toShowSalaryDetail.action?id="+res[i].id+"&time="+time+"'>详情</a>"};
