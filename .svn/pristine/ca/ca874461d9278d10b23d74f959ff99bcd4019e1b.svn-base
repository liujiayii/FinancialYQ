		...
		<div class="table-frame pdt60">
			...
			<div class="left">
				...
				<table class="receipt-table" cellpadding="0" cellspacing="0" width="890px;" border="1">
					...
					<c:choose>
						<c:when test="${ empty finance.refinancePlatform}">
						</c:when>
						<c:otherwise>
						<tr>
							<td>续贷还平台</td>
							<td colspan="8" id="text1">${finance.refinancePlatform}</td>
						</tr>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${ empty finance.maturityPlatform}">
						</c:when>
						<c:otherwise>
						<tr>
							<td>到期还平台</td>
							<td colspan="8" id="text2">${finance.maturityPlatform}</td>
						</tr>
						</c:otherwise>
					</c:choose>
					<c:choose>
