...
<div class="popup">
	<form id="ff">
		<div class="popup-layer">
			<input type="hidden" name="id" value="" /> <span>标号</span><input type="text" name="grade" id="area" class="popup-input" onkeyup="findGread()" />

		</div>
		<p class="popup-error" id="area-error"></p>
		<div class="popup-layer">
			<span>逾期</span> <select class="popups-select" name="is_overdue" id="is_overdue">
				<option value="0">否</option>
				<option value="1">是</option>
			</select>
		</div>
		<div class="popup-layer">
			<span>提前还款</span> <select class="popups-select" name="is_payment" id="is_payment">
				<option value="0">否</option>
				<option value="1">是</option>
			</select>
		</div>
		...
		<div class="popup-layer" style="text-align: center;">
			<input type="button" class="popup-sub" value="保存" onclick="update()" /> <input type="button" value="取消" class="popup-return" />
		</div>
	</form>
</div>
</body>
			...
			$.getJSON(url, function(res) {
				...
				for (var i = 0; i < res.length; i++) {
					data[i] = {
						...
						money : res[i].money,
						day : res[i].day,
						is_payment : res[i].is_payment == 0 ? '<span style="color:green">否</span>' : '<span style="color:red">是</span>',
						is_overdue : res[i].is_overdue == 0 ? '<span style="color:green">否</span>' : '<span style="color:red">是</span>',
